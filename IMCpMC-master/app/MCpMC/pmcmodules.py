...
class PmcModules:
    ...
    def get_reward(self, act):
        ...
        for rew_action, cond, reward in self.reward:
            # print("rew", substitution, "cond", cond, "sub", mysub(cond, substitution), "reward", reward)
            # print("____")
    ...
    def get_valuation(self):
        """ return the current value of all variable (global+sates in modules)"""
        # print("sum", self.current_value_global)
        return {**dict(i for mod in self.modules for i in mod.current_value_state.items()), **self.current_value_global}
